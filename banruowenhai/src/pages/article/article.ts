...
export class ArticlePage {
  ...
  private initializePage(bookName, volume, volumes, volumeIndex, scrollPosition){
    ...
    let sectionKey = regexp.exec(this.volume.url);    
    this.sectionArray = this.globalVars.getSectionsDict()[sectionKey.toString()];
    for(let section of this.sectionArray){
      console.debug("Section Name:" + section.name + ", href:" + section.href);
    }
  ...
  private getContent(plt: Platform): Observable<any> {
    ...
    if(plt.is("android")){
      this.url = "/android_asset/www/" + this.volume.url.replace(".htm",".txt").replace("../","");
    }else if(plt.is("ios")){
      this.url = this.volume.url.replace(".htm",".txt").replace("../","");
    }else{
      this.url = this.volume.url.replace(".htm",".txt");
    }
  ...
  saveHistory(){
    ...
    let history = new History(this.volume.url, this.volume.name, this.docScollPosition, (this.docScollPosition==0 || this.docScollPosition==null)?0:Math.round(100*(this.docScollPosition/this.docFullHeight)), this.volume, this.volumes, this.volumeIndex);
