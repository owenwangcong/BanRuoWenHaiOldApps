...
export class GlobalVars {
  ...
  public static TRACKER_ID:string = "UA-111883791-1";
  ...
  constructor(private http: Http, private storage:Storage, private storageServices:StorageServices, private splashScreen:SplashScreen) {
    ...
    this.parseCategoryDict().subscribe(data => {
        ...
        console.log("Total categoryDicts loaded:" + this.categoryDict.length);
    ...
    this.parseBooks().subscribe(data => {
      this.bookList = data;
      console.log("Total books loaded:" + this.bookList.length);
      this.checkAndHideSplash();
    }, error => console.error(error));

    //Load section dict
    this.parseSectionsDict().subscribe(data => {
        this.sectionsDict = data
        console.log('Total sectionsDicts loaded:' + this.sectionsDict.length);
        this.checkAndHideSplash();
      }, error => console.error(error));  
  ...
  private parseSectionsDict(): Observable<any> {
    return this.http.get("input/sectionsDict.json")
                    .map((res:any) => res.json())
                    .catch((error:any) => Observable.throw(error.json().error || 'Server error'));
  }
