...
export class ArticlePage {
  ...
  private initializePage(bookName, volume, volumes, volumeIndex, scrollPosition){
    ...
    if(this.volumeIndex>0 && (this.volumes[this.volumeIndex-1].url!=null && this.volumes[this.volumeIndex-1].url!="")){
    ...
    if(this.volumeIndex+1<this.volumes.length && (this.volumes[this.volumeIndex+1].url!=null && this.volumes[this.volumeIndex+1].url!="")){
  ...
  private getContent(plt: Platform): Observable<any> {
    ...
    var regExE = /=/gi; 
    var regExA = /&/gi; 
    this.url = this.volume.url.replace("http://www.zhibeifw.com/","");
