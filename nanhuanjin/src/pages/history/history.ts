...
presentDeleteAllConfirm() {
  ...
  gotoArticle(history:History) {
    var bookName = this.getBookName(history);
    this.navCtrl.push(ArticlePage, {bookName:bookName, volume:history.volume, volumes:history.volumes, volumeIndex:history.volumeIndex, scrollPosition:history.scrollPosition});
  }

  getBookName(history:History){
    var bookName;
